...
void processCommand(String cmd) {
  ...
  if (cmd.indexOf(',') > 0 && cmd.indexOf(':') == -1) {
    ...
    if (secondComma > firstComma) {
      ...
      emergencyStop = false;  // Clear emergency stop when new command received
  ...
  if (now - lastUltrasonicTime >= ULTRASONIC_PERIOD) {
    ...
    if (distance < STOP_DISTANCE_CM) {
      if (!emergencyStop) {
        ...
        Serial.println(" cm");
        stopAllMotors();
        // Send emergency stop status via UART2
        Serial2.println("ESTOP:ACTIVE");
      }
    } else if (emergencyStop && distance > STOP_DISTANCE_CM + 10) {
      // Clear emergency stop when object is far enough (with hysteresis)
      emergencyStop = false;
      Serial.println("Emergency stop cleared");
      // Send emergency stop cleared status via UART2

...
//TBT
/*  
Motor controller with ultrasonic sensor - MODIFIED VERSION
- Removed velocity reporting, now sends distance data
- Distance sent every 100ms in format: "DIST:XXX.XX,timestamp\n"
...
float currentDistance = 999.0;  // Store current filtered distance
...
void setup() {
  ...
  Serial.println("Motor controller with ultrasonic distance reporting ready");
  Serial.println("UART2 on GPIO16(RX), GPIO17(TX)");
  Serial.println("Ultrasonic on GPIO26(TRIG), GPIO27(ECHO)");
  Serial.println("Now sending distance data instead of velocity");
...
void loop() {
  ...
  if (now - lastUltrasonicTime >= ULTRASONIC_PERIOD) {
    ...
    currentDistance = distance;  // Store the current distance
  ...
  if (now - lastTime >= 100) {
    ...
    // Send distance data format: "DIST:distance,timestamp\n"
    // This replaces the old velocity data sending
    Serial2.print("DIST:");
    Serial2.print(currentDistance, 2);
    Serial2.print(",");
    ...
    // Display status (now includes distance instead of velocities)
    Serial.print("Distance:"); Serial.print(currentDistance, 1); Serial.print("cm");
